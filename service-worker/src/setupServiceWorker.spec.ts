...
describe("setupServiceWorker", () => {
  ...
  let mockGlobalCaches: CacheStorage;
  beforeEach(() => {
    mockServiceWorker = {
  ...
  it("caches initial files on install", async () => {
    ...
    const installHandler = (
      mockServiceWorker.addEventListener as jest.Mock
    ).mock.calls.find((call: unknown[]) => call[0] === "install")[1];
  ...
  it("should handle fetch requests with cache-first strategy", async () => {
    ...
    const fetchHandler = (
      mockServiceWorker.addEventListener as jest.Mock
    ).mock.calls.find((call: unknown[]) => call[0] === "fetch")[1];
  ...
  it("should clean up old caches on activate", async () => {
    when(mockGlobalCaches.keys).mockResolvedValue(["old-cache", "new-cache"]);

    setupServiceWorker(mockServiceWorker, "new-cache", "irrelevant");

    const activateHandler = (
      mockServiceWorker.addEventListener as jest.Mock
    ).mock.calls.find((call: unknown[]) => call[0] === "activate")[1];
