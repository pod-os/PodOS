...
describe('pos-login-form', () => {
  ...
  it('renders an input for the idpUrl and a login button', async () => {
    const page = await render(<pos-login-form></pos-login-form>);
    ...
    expect(page.root.shadowRoot).toEqualHtml(`
      ...
      <input id="idpUrl" type="url" required list="suggestedIssuers" placeholder="Type to search...">
  ...
  describe('enabling and disabling the login button', () => {
    it('is disabled initially', async () => {
      const page = await render(<pos-login-form></pos-login-form>);
      const button: HTMLButtonElement = withinShadow(page).getByRole('button');
  ...
  it('pre-fills IdP URL and checks the box when a value is in settings', async () => {
    ...
    const idpInput: HTMLInputElement = withinShadow(page).getByLabelText('Please enter your Identity Provider');
    expect(idpInput.value).toBe('https://remembered.idp.test');

    // and the checkbox is pre-checked
    const checkbox: HTMLInputElement = withinShadow(page).getByLabelText('Remember me');
    expect(checkbox.checked).toBe(true);

    // and the submit button is already enabled
    const button: HTMLInputElement = withinShadow(page).getByRole('button');
    expect(button.disabled).toBe(false);
  });
  ...
  it('saves IdP URL to settings on submit when "Remember me" is checked', async () => {
    ...
    const form: HTMLFormElement = getForm(page);
...
function getUrlInput(page: RenderResult<HTMLElement, any>) {
  return page.root.shadowRoot!.querySelector('input')!;
}

function getForm(page: RenderResult<HTMLElement, any>) {
  return page.root.shadowRoot!.querySelector('form')!;
}
